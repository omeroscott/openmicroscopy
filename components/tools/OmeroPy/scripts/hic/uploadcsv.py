...
def cleanString(input):
	"""
	Replaces spaces and colons in the input string with underscores or dashes. This is useful when generating filenames because sane filenames don't contain certain characters, such as spaces, and some characters are treated differently by different filesystems, for example colons don't play nicely on OS X and have to be escaped.
...
def createDataset(parent=None, name=None, date=False):
	...
	Arguments:
		...
		parent
			The omero.model.ProjectI object to link this dataset to. If parent is none then an orphan dataset is created.
		date
			If a name is not supplied and date=False then a uuid is generated and used as the name. If a name is not supplied and date=True then the current date and time are used to generate a name.
...
#		print dataset.getId().getValue()
		...
		fileAnnotation = scriptUtil.uploadAndAttachFile(queryService, updateService, rawFileStore, dataset, filepath, mimetype)
